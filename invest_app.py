...
def dynamic_settings(label, prefix, default_val, is_rate=False, is_withdrawal=False):
    with st.sidebar.expander(label):
        ...
        w_mode = "定額 (円)"
        if is_withdrawal:
            w_mode = st.radio(f"計算方法", ["定額 (円)", "定率 (%)"], 
                              index=0 if get_p(f"{prefix}_m", "定額 (円)") == "定額 (円)" else 1, key=f"{prefix}_m")
        ...
        for i in range(count):
            ...
            if is_rate or (is_withdrawal and w_mode == "定率 (%)"):
                val = float(col1.number_input(f"値", -15.0, 30.0, float(get_p(f"{prefix}_v{i}", default_val)), step=0.1, key=f"{prefix}_v{i}"))
            else:
                val = int(col1.number_input(f"値 (円)", 0, None, int(get_p(f"{prefix}_v{i}", int(default_val))), step=10000, key=f"{prefix}_v{i}"))
            
            age = int(col2.number_input(f"開始年齢", min_age_for_row, end_age, int(max(min_age_for_row, get_p(f"{prefix}_a{i}", min_age_for_row))), key=f"{prefix}_a{i}"))
...
with st.sidebar.expander("🏥 臨時収支の設定"):
    ...
    for i in range(exp_count):
        c1, c2 = st.columns(2)
        v = int(c1.number_input(f"金額{i+1}(円)", value=int(get_p(f"ev{i}", 0)), step=100000, key=f"ev{i}"))
        a = int(c2.number_input(f"年齢{i+1}", current_age, end_age, int(max(current_age, get_p(f"ea{i}", current_age))), key=f"ea{i}"))
...
def run_simulation():
    ...
    for m in range(1, total_months + 1):
        ...
        def find_v(s_list, target_age):
            res_v, res_m = s_list[0]["val"], s_list[0].get("mode", "定額 (円)")
            for s in s_list:
                if target_age >= s["age"]:
                    res_v, res_m = s["val"], s.get("mode", "定額 (円)")
            return res_v, res_m

        ann_rate, _ = find_v(rates, p_age)
        event_val = float(event_dict.get(m, 0))
        balance += event_val
        if event_val > 0: cum_inv += event_val

        w_val, w_mode = find_v(withdrawals, p_age)
        d_val, _ = find_v(deposits, p_age)
        
        # 取り崩し開始年齢に達しているか判定
        first_w_age = min([s["age"] for s in withdrawals if s["val"] > 0], default=999)
        
        m_flow, action = 0.0, "待機"
        if p_age > first_w_age:
            if w_mode == "定額 (円)": m_flow = -float(w_val)
            else: m_flow = -(balance * (float(w_val) / 100)) / 12
            action = "取り崩し"
        elif p_age >= deposits[0]["age"]:
            m_flow = float(d_val)
            cum_inv += m_flow
            action = "積立"
            
        balance = max(0.0, balance + m_flow) * (1 + (float(ann_rate)/100) / 12)
        data.append({
            "年齢(グラフ)": round(p_age, 2), "年齢": display_age, "月": f"{(m-1)%12+1}ヶ月", 
            "区分": action, "月間収支": int(m_flow), "臨時収支": int(event_val), 
            "元本": int(cum_inv), "資産残高": int(balance)
        })
