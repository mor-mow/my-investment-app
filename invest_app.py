...
end_age = int(st.sidebar.slider("終了年齢", current_age, 100, int(max(current_age + 1, get_p("end", 85))), key="end"))
...
withdrawals_list = dynamic_settings("🚪 取り崩し設定", "wd", 0, is_withdrawal=True) # 初期値を0に
...
# --- 6. 表示エリア（気分を害さないガード付き） ---
is_input_empty = (initial_sum == 0 and not any(s["val"] > 0 for s in deposits_list))

if is_input_empty:
    st.info("👋 シミュレーションを開始するには、左側のメニューから「現在の一括投資額」や「積立設定」を入力してください。")
    st.image("https://unsplash.com", caption="あなたの将来をシミュレーションしましょう")
else:
    if not df.empty:
        c1, c2, c3 = st.columns(3)
        last_bal = df.iloc[-1]['資産残高']
        c1.metric(f"{end_age}歳時点の資産", f"¥{last_bal:,}")
        c2.metric("全体の平均年率", f"{avg_r:.2f}%")
        
        if last_bal <= 0:
            c3.warning("⚠️ 途中で見直しが必要かも")
        else:
            c3.success("✅ 順調なプランです")
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df_g["年齢(グラフ)"], y=df_g["資産(万円)"], name="資産残高", line=dict(color="#1f77b4", width=3), fill='tozeroy', fillcolor='rgba(31, 119, 180, 0.1)'))
        ...
        fig.update_xaxes(title="年齢", showgrid=True, gridcolor="rgba(128,128,128,0.2)", zeroline=True)
        fig.update_yaxes(title="万円", ticksuffix="万", tickformat=",", showgrid=True, gridcolor="rgba(128,128,128,0.2)", zeroline=True)
