...
with st.sidebar.expander("🏥 臨時収支の設定"):
    st.caption("プラスは収入(退職金等)、マイナスは出費")
...
def run_simulation():
    ...
    for m in range(1, total_months + 1):
        ...
        def find_v(s_list, target_age):
            ...
            for s in s_list:
                if target_age >= s["age"]: res_v, res_m = s["val"], s.get("mode", "定額 (円)")
            return res_v, res_m

        ann_rate, _ = find_v(rates, p_age)
        ev_val = float(event_dict.get(m, 0))
        balance += ev_val
        if ev_val > 0: cum_inv += ev_val # 臨時収入は元本に加算

        w_val, w_mode = find_v(withdrawals, p_age)
        d_val, _ = find_v(deposits, p_age)
        
        # 取り崩し判定（全設定の中で最初の取り崩し年齢以降）
        is_w_period = any(s["val"] > 0 and p_age >= s["age"] for s in withdrawals)
        m_flow, action = 0.0, "待機"
        if is_w_period:
            if w_mode == "定額 (円)": m_flow = -float(w_val)
            else: m_flow = -(balance * (float(w_val) / 100)) / 12
            action = "取り崩し"
        elif any(s["val"] > 0 and p_age >= s["age"] for s in deposits):
            m_flow = float(d_val)
            cum_inv += m_flow
            action = "積立"
...
# --- 6. URLクエリパラメータ更新 (安定化のため全ステートを同期) ---
